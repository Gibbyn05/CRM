...
//
//  Brønnøysundregisteret har ikke telefonnummer, så for det feltet kan man
//  koble på en av de kommersielle nummeropplysningstjenestene. Ingen av
//  1881/Gule Sider/180.no har en fri, offentlig dokumentert API for dette –
//  alle tre krever en kommersiell avtale/API-nøkkel. Vi bygger derfor et
//  rent, utskiftbart grensesnitt (CompanyLookupProvider) og lar hver
//  leverandør stå AV (dry-run/ikke konfigurert) helt til noen limer inn en
//  ekte nøkkel + evt. riktig base-URL for avtalen sin.
//
...
//  tatt – oppslaget markerer bare feltet som "ikke tilgjengelig".
//
...
//  faktiske API-kontrakt (endepunkt/auth/respons-format varierer).
// ============================================================================
...
function makeStubPhoneProvider(opts: {
  ...
  return {
    id: opts.id,
    isConfigured: () => Boolean(process.env[opts.apiKeyEnv]),
    ...
    async lookupPhone(orgNumber: string): Promise<string | null> {
      ...
      try {
        ...
        // TODO: bytt ut med ${opts.label} sin faktiske API-kontrakt når
        // avtale/dokumentasjon er på plass.
